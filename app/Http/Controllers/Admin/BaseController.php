...
class BaseController extends Controller
{
    ...
    protected function getLanguageSwitchBarHtml()
    {
        $locales = [
            'zh-CN' => '简体中文',
            'en' => 'English',
        ...
        foreach ($locales as $langCode => $langName) {
            $cssClass = \App::getLocale() == $langCode ? 'active' : '';
            $params = \Request::input();
            $params['_locale'] = $langCode;
            $paramsString = http_build_query($params);
            $localeSwitchHtml = '<a href="?'.$paramsString.'" class="btn btn-link'.$cssClass.'">'.$langName.'</a> ';
        }
        return $localeSwitchHtml;
    }
